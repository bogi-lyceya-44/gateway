...
VENDOR_DIR=vendor.protogen/github.com/bogi-lyceya-44/task-tracker
VENDOR_PROTO_CFG=easyp.vendor.yaml
LOCAL_PROTO_CFG=easyp.yaml
...
.generate:
	...
	(PATH="$(PATH):$(LOCAL_BIN)" && $(EASYP_BIN) -cfg $(VENDOR_PROTO_CFG) mod download && $(EASYP_BIN) -cfg $(VENDOR_PROTO_CFG) generate)

	(PATH="$(PATH):$(LOCAL_BIN)" && $(EASYP_BIN) -cfg $(LOCAL_PROTO_CFG) mod download && $(EASYP_BIN) -cfg $(LOCAL_PROTO_CFG) generate)
